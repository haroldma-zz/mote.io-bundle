...
window.MoteioReceiver = function() {
  ...
  self.listen = function(username) {
    ...
    self.channel.bind('client-got-config', function(){
    ...
    self.channel.bind('client-go-home', function(){
      self.goHome();
    });

    self.channel.bind('client-input', function (data) {
      self.clog('Got Button Input')
      self.triggerInput(data);
    });

    self.channel.bind('client-select', function (data) {
    ...
    self.channel.bind('client-search', function (data) {
      self.clog('Got Search Input');
      self.params.blocks[data.block_id].action(data.query);
    });

    self.channel.bind('client-deactivate', function () {
  			...
  			$.each(self.params.blocks[i].data, function(j, block_data) {
  				...
  				if(self.params.blocks[i].data[j].hash == hash) {
				    ...
				    if (self.channel && worthUpdating) {
				    	// console.log('worthUpdating')
				    	// console.log(self.params.blocks[i].data[j])
				      self.channel.trigger('client-update-button', self.params.blocks[i].data[j]);
				    }
