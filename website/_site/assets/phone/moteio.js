...
var App = function () {
  ...
  self.listen = function (uid) {
    ...
    self.channel.emit('get-config', uid, function (err, res) {
      ...
      if (err) {
        ...
        for (button_id in res.buttons) {
          ...
          wrapper = $('<div class="moteio-placement moteio-button"></div>')
            .css({
              'left': res.buttons[button_id].x,
              'top': res.buttons[button_id].y,
              'position': 'absolute'
            });

          element = $('<a href="#" id="button-'+button_id+'" class="icon-' + res.buttons[button_id].icon + '" /></a>')
    ...
    self.channel.on('change-button', function (data) {
      console.log('got button change')
      if(data.icon){
        $('#button-' + data.id).removeClass().addClass('icon-' + data.icon);
      }
      if(data.color){
        $('#button-' + data.id).css({color: data.color});
      }
    });
